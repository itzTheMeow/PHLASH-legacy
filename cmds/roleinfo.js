...
module.exports.run = async (client, message, args) => {
  ...
  if (!role) {
    role = message.guild.roles.cache.filter((role) =>
      role.name.toLowerCase().includes(args.slice(1).join(" ").toLowerCase())
    );
    if (role.size == 0)
      return message.channel.send("I can't find any roles that match that search!");
    if (role.size >= 2)
      return message.channel.send(
        "Hmm... I found multiple results for that role. Try being more specific?"
      );
    role = role.first();
  }
